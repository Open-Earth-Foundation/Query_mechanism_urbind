...
def test_estimate_context_window_reports_full_catalog_tokens_before_split() -> None:
    ...
    assert estimate.context_window_tokens == count_tokens(
        context_chat._render_citation_catalog_block(citation_catalog)
    )
...
def test_plan_context_chat_request_prefers_direct_mode_for_small_prompt() -> None:
    ...
    assert plan.history_tokens == context_chat._estimate_messages_tokens([])
...
def test_run_overflow_evidence_map_reduce_logs_split_mode(
    ...
    monkeypatch.setattr(context_chat, "_resolve_prompt_budget", lambda **_kwargs: ([], 120))
    monkeypatch.setattr(
        context_chat,
        "_build_request_evidence_blocks",
    ...
    result = context_chat._run_overflow_evidence_map_reduce(
