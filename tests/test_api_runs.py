...
def test_api_writer_context_export_returns_writer_safe_markdown_bundle(tmp_path: Path) -> None:
    runs_dir = tmp_path / "output"
    markdown_dir = tmp_path / "documents"
    markdown_dir.mkdir(parents=True, exist_ok=True)
    config = _build_config(runs_dir=runs_dir, markdown_dir=markdown_dir)
    ...
    assert "- Analysis mode: aggregate" in payload
    assert "- Selected cities: Munich, Leipzig" in payload
    assert "- Excerpt count: 2" in payload
    assert "## SQL Context" in payload
    assert '"city": "Munich"' in payload
    assert "## Excerpt 1 - Munich (`ref_1`)" in payload
    assert "## Excerpt 2 - Leipzig (`ref_2`)" in payload
    assert "> Munich is retrofitting schools." in payload
    assert "**Source chunk ids:** `chunk_munich_1`" in payload
