...
from backend.modules.writer.models import WriterEvidenceSelection, WriterSectionPlan
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            ("WriterSectionPlan", "`section_id`", "`writing_instructions`", "`saved_evidence_catalog`"),
        ),
        (
        ...
        (
            "writer_research_curator_system.md",
            ("WriterEvidenceSelection", "`context_source_summary`", "`regex_search_context`"),
        ),
    ],
)
...
def test_writer_research_curator_prompt_example_matches_runtime_schema() -> None:
    prompt_path = Path("backend/prompts/writer_research_curator_system.md")
    content = prompt_path.read_text(encoding="utf-8")
    example = content.split("<example_output>", 1)[1].split("</example_output>", 1)[0]

    parsed = WriterEvidenceSelection.model_validate_json(example)

    assert parsed.status == "saved_evidence"
    assert parsed.saved_evidence_ids == ["ws_1", "ws_2"]
