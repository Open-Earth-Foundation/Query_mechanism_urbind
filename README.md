...
- `markdown/thrown_excerpts.json`: rejected markdown excerpts captured during extraction. Includes `thrown_excerpt_count` and `thrown_excerpts` entries with rejection metadata (`rejection_stage`, `reason_codes`, `batch_index`, `expected_city_name`, and optional `invalid_source_chunk_ids`) for debugging.
