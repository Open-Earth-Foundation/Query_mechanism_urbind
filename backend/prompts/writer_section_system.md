...
- `context_bundle` (object): writer-safe context slice for this section.
  ...
  - `markdown.excerpts` (list[object]): assigned evidence excerpts with `ref_id`, `city_name`, `quote`, `partial_answer`, and optional saved-evidence provenance fields such as `source_kind`, `source_id`, `field`, `line_start`, `line_end`, and `writer_saved_id`.
...
- Saved external, web, assumption, source-chunk, or enrichment records in `markdown.excerpts` are citation-compatible writer evidence and must use their assigned `[ref_n]`.
