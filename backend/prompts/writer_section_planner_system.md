...
- `evidence_catalog` (list[object]): compact writer evidence records available to the writer.
  ...
  - `source_kind` (str): `ccc_excerpt`, `ccc_source_chunk`, `external_markdown_claim`, `external_markdown_resolution`, `web_finding`, `assumption`, `non_estimable`, `enriched_field`, `freshness_result`, or similar writer evidence kind.
...
- `required_ref_ids` (list[str]): `ref_n` ids the section must use. Use only ids present in `evidence_catalog`.
...
- Preserve the chapter approach: plan sections only; prose is written by section writers.
