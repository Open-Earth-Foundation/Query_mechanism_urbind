...
- `selected_run_ids` (list[str]): currently selected base run ids.
- `selected_followup_bundle_ids` (list[str]): currently attached follow-up bundle ids.
- `contexts` (list[object]): compact context summaries. Each item has:
  - `source_type` (str): `run` or `followup_bundle`.
  - `source_id` (str): run id or bundle id.
