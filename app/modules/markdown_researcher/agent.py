...
def extract_markdown_excerpts(
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if attempt < max_retries and _is_retryable_error(exc):
                    logger.warning(
                        "Markdown %s batch %s failed (attempt %d/%d); retrying.",
                        city_name,
                        batch_index,
                        attempt + 1,
                        max_retries + 1,
    ...
    for city_name, city_documents in sorted(documents_by_city.items()):
        ...
        batches = split_documents_by_token_budget(city_documents, max_input_tokens)
        if len(batches) > 1:
            logger.info("Split %s into %s batches", city_name, len(batches))
        for batch_index, batch in enumerate(batches, start=1):
            all_tasks.append((city_name, batch_index, batch))

    # Process all city batches in parallel
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {
            executor.submit(_process_city_batch, city_name, batch_idx, batch): (
                city_name,
                batch_idx,
            )
